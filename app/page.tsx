...
import { Sparkles, Package, Shield, Zap, TrendingUp, History, RotateCcw } from 'lucide-react';

export default function HomePage() {
  const { balance, level, xp, addBalance, resetEconomy, isLoaded } = useEconomy();
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-16">
      ...
      <section className="text-center pt-8 pb-12">
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/30 text-accent text-xs font-bold uppercase tracking-wider mb-6"
        >
        ...
        <motion.h1
          ...
          className="text-5xl sm:text-7xl md:text-8xl font-display font-black tracking-tighter mb-6 uppercase"
        >
        ...
        >
          Authentic weighted drop chances, realistic CS2 skin collections, interactive roulette spin animations, and instant inventory management.
        </motion.p>
        ...
        <div className="flex flex-wrap justify-center items-center gap-4">
          ...
          <button
            onClick={() => {
              if (confirm('Reset your balance, XP, and inventory to starting defaults?')) {
                resetEconomy();
              }
            }}
        ...
        {inventory.length === 0 ? (
          <div className="text-center py-16 bg-surface-dark/50 rounded-3xl border border-dashed border-white/15">
            ...
            <p className="text-xs text-text-muted mt-1 max-w-sm mx-auto">
              Open any case above to begin collecting rare weapons and building your inventory valuation!
            </p>
          </div>
        ) : (
